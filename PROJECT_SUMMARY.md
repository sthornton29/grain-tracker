...
>
> _Snapshot date: 2026-07-13 (evening). Schema at migration `042`._
...
| Testing / CI | **Vitest `^4.1.8`** — 583 unit tests over the pure `lib/` math (21 `lib/*.test.ts` files); ESLint (`.eslintrc.json`, `next/core-web-vitals`); **GitHub Actions CI** (`.github/workflows/ci.yml`: lint + test on every push/PR) |
...
plus a **Cotton** item (with a Seed Cotton Loads / Gin Receipts / Bales & Grades sub-nav) when
the **Cotton module** is enabled (`app_settings.cotton_module_enabled`, toggled under
Settings → Users & Modules). **Gin-role** users see ONLY the three Cotton pages.
...
| `/reports/season` | **Season Summary** — acres & yield by crop; excludes unharvested/in-progress. A flag-gated **Cotton (lint) section** (`components/reports/cotton-yields-section.tsx`, also on `/yields`) shows lbs lint/acre, seed cotton lbs/ac, turnout %, and "on yard awaiting gin" per field. |
...
**Supabase / PostgreSQL** with `pgcrypto` (`gen_random_uuid()`). Defined by **42 sequential,
idempotent migrations** in `supabase/` (`schema.sql` = 001, then `002_*.sql` … `042_*.sql`).
Every table re-runs safely (`create table if not exists`, guarded `do $$…$$`), uses a `uuid`
PK and `created_at`. ~46 tables, **no views**. Later migrations frequently `ALTER` earlier
...
- **Status:**
  - Futures quotes: **live, fault-tolerant.** ICE Cotton (CT…) quotes stay in **¢/lb as quoted** (72.65) — no ÷100; grain quotes convert cents/bu → $/bu. Without the key (or on error / market-closed), every route
...
- **Cotton prices are ¢/lb end to end** (stored 72.65, not 0.7265): futures P&L must use `pnlSizeFor()` ($500/point), never the raw 50,000-lb contract size; the Barchart route deliberately skips the ÷100 for CT symbols.
- **Cotton scope notes**: physical cotton marketing/contracts are deliberately NOT tracked yet (hedging + production only); the Crop Insurance Production report and the Yields variety/landowner groupings don't yet include cotton (it has its own unit-aware section on Yields/Season instead).
