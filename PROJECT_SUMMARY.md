...
>
> _Snapshot date: 2026-07-09. Schema at migration `038`._
...
| Testing / CI | **Vitest `^4.1.8`** — 511 unit tests over the pure `lib/` math (17 `lib/*.test.ts` files); ESLint (`.eslintrc.json`, `next/core-web-vitals`); **GitHub Actions CI** (`.github/workflows/ci.yml`: lint + test on every push/PR) |
...
| `/reports/government-payments` | **Government Payment Tracker** — projected ARC/PLC + other USDA payments with per-entity payment-limit tracking. A shared **MYA Prices panel** (`components/reports/mya-price-panel.tsx`, also on the Decision Aid) shows each commodity's resolved MYA with a state chip (**est. / manual / final**), its source (live Barchart / stored / operator override / USDA final), the effective reference price, and the **PLC payment-rate spread** (reference − MYA floored at 0) — plus an **Auto \| Manual toggle** (Manual persists an inline price to `arc_plc_price_data` with `source='manual'`; Auto reverts and clears it; untraded commodities are manual-only; a published final locks the row). The Details expander's monthly table has a **"Look up USDA prices"** button per commodity × marketing year (`/api/mya-monthly-lookup` — Claude web search over NASS monthly prices for the elapsed marketing-year months): fetched months render beside existing entries with differences highlighted, months already entered start **unchecked** (never overwritten without explicit confirmation), confirmed months save with `source='ai'` (an edited value becomes `'manual'`; typed entries save as `'manual'`), and the blend + "N of 12 months USDA-published" composition line recompute on save. All projections update live on toggle. Exports carry the on-screen per-commodity **Election** (PLC/ARC-CO) column and the **Payment Limit Status** table (second section, status tones). |
...
**Supabase / PostgreSQL** with `pgcrypto` (`gen_random_uuid()`). Defined by **38 sequential,
idempotent migrations** in `supabase/` (`schema.sql` = 001, then `002_*.sql` … `038_*.sql`).
