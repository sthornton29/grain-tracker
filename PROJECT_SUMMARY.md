...
>
> _Snapshot date: 2026-07-19. Schema at migration `042`._
...
| Testing / CI | **Vitest `^4.1.8`** — 619 unit tests over the pure `lib/` math (23 `lib/*.test.ts` files); ESLint (`.eslintrc.json`, `next/core-web-vitals`); **GitHub Actions CI** (`.github/workflows/ci.yml`: lint + test on every push/PR) |
...
| `/settings/varieties` | **Varieties duplicate cleanup** — per crop, suspected-duplicate spellings (normalized-equal or near keys, union-find clustered, sorted most-used first) with per-planting usage counts; pick the canonical spelling and **Merge**: every planting keeps exactly one row renamed to the canonical name with acres/bushels summed (`buildVarietyMergePlan`), redundant rows deleted. |
...
- **Variety resolution pipeline** (`lib/variety-resolution.ts`) — pure, shared by the CSV importer, the AI plantings import, and Settings → Varieties. `varietyKey` normalizes spellings (uppercase, spaces/hyphens/periods stripped) so format variants ("DG 3644 B3XF" / "dg3644-b3xf") are the SAME variety and auto-link to the stored spelling; `keysNear` flags close-but-different names (substring ≥4 chars, matching digit-tail — seed identity lives in the number+trait tail, or edit distance ≤2) as **possible matches the user must decide** — never auto-merged, since "DG 3644" vs "DG 3646" are different products; genuinely new names are created once per file (first-seen spelling becomes canonical, variants collapse via `buildVarietyPlan`). `findSimilarVarietyGroups` (union-find clustering) + `buildVarietyMergePlan` (one surviving row per planting, acres/bushels summed) power the duplicate-cleanup screen.
...
| `plantings` | `<PlantingsAiImport>` | Per planting: field, crop, season_year, planted/irrigated acres, planting date, `varieties[]`, notes. Classifies New/Update/Unchanged. Variety names run through the shared **variety-resolution pipeline** (per crop): exact-key matches auto-link to the stored spelling, near names block save until the user picks link-vs-create, new names create once per file. |
