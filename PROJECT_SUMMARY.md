...
| Testing / CI | **Vitest `^4.1.8`** — 345 unit tests over the pure `lib/` math (12 `lib/*.test.ts` files); ESLint (`.eslintrc.json`, `next/core-web-vitals`); **GitHub Actions CI** (`.github/workflows/ci.yml`: lint + test on every push/PR) |
...
| `/reports/crop-insurance-claims` | **Crop Insurance Claims Monitor** — estimated indemnity per policy (RP/RP-HPE/YP + SCO/ECO) with What-If sliders. **Per-practice**: irrigated and dryland are separate rows, each using its own practice yield from the `irrigated_bushels`/`dryland_bushels` breakout (crop-default fallback), with a per-crop subtotal. Links to the **Coverage Check** (`/settings/crop-insurance#coverage-check`) so insured-vs-planted reconciliation is reachable from where insurance work happens. |
