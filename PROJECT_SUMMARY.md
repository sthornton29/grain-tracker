...
>
> _Snapshot date: 2026-06-15. Schema at migration `034`._
...
| Testing / CI | **Vitest `^4.1.8`** — 307 unit tests over the pure `lib/` math (12 `lib/*.test.ts` files); ESLint (`.eslintrc.json`, `next/core-web-vitals`); **GitHub Actions CI** (`.github/workflows/ci.yml`: lint + test on every push/PR) |
...
- **Insurance / government / revenue engines** (`lib/crop-insurance.ts`, `lib/government-payments.ts`, `lib/revenue-projections.ts`) — pure math feeding the financial reports. Projected prices now come from `harvest_price_estimates` (`projectedPriceFromEstimates`) instead of a hard-coded map. `computeRevenueProjections` **consumes the marketing engine's `blendedRevenue`** for crop sales and layers insurance proceeds + government payments on top; its **breakeven = cost ÷ yield / cost ÷ avg price** mirrors the Marketing dashboard (sales-only).
- **Unit tests** — 307 Vitest tests across 12 `lib/*.test.ts` files (shrink, yields, csv, contracts, marketing, crop-insurance, government-payments, revenue-projections, **revenue-marketing-reconciliation**, hedging, load-splits, program-config) with hand-verified worked examples; run in CI on every push/PR. The reconciliation file asserts the identity *Revenue Projections profit − Marketing total profit = insurance + government* per crop.
...
| `brokerage_statement` | `<StatementImport>` (hedging) | RJ O'Brien futures/options: long/short, open/closed trades, options, account summary. After review, a **second step** flags app-open positions missing from the statement as *possibly closed* → Close (real workflow) / Keep open. |
