...
>
> _Snapshot date: 2026-07-13. Schema at migration `041`._
...
| Testing / CI | **Vitest `^4.1.8`** — 570 unit tests over the pure `lib/` math (20 `lib/*.test.ts` files); ESLint (`.eslintrc.json`, `next/core-web-vitals`); **GitHub Actions CI** (`.github/workflows/ci.yml`: lint + test on every push/PR) |
...
**Supabase / PostgreSQL** with `pgcrypto` (`gen_random_uuid()`). Defined by **41 sequential,
idempotent migrations** in `supabase/` (`schema.sql` = 001, then `002_*.sql` … `041_*.sql`).
Every table re-runs safely (`create table if not exists`, guarded `do $$…$$`), uses a `uuid`
PK and `created_at`. ~38 tables, **no views**. Later migrations frequently `ALTER` earlier
...
# In the Supabase SQL editor, run supabase/schema.sql, then 002…041 in order
