...
>
> _Snapshot date: 2026-07-09. Schema at migration `039`._
...
| AI | **`@anthropic-ai/sdk ^0.95.2`** — Claude `claude-sonnet-4-6` for document extraction + two **web-search lookups** (ARC-CO benchmarks, USDA monthly MYA prices) via shared `lib/ai-web-search.ts` |
...
| Testing / CI | **Vitest `^4.1.8`** — 522 unit tests over the pure `lib/` math (17 `lib/*.test.ts` files); ESLint (`.eslintrc.json`, `next/core-web-vitals`); **GitHub Actions CI** (`.github/workflows/ci.yml`: lint + test on every push/PR) |
...
| `/settings/government-payments` | ARC/PLC base acres, elections, price data (incl. **WASDE midpoint**), **ARC-CO Benchmarks** (FSA benchmark price/county yield per commodity × **county + state** — rows key on `county_id` and display as **"County, ST"** since county names repeat across states; picked via a **State → County cascade** (two selects; the county list is the FULL state list from the paginated counties fetch, with the operation's own counties in a "Your counties" group on top); manual or **AI autofill** via `/api/arc-benchmark-lookup` with user confirmation — the lookup **requires county + state**, is cached on commodity + county_id + program year, **falls back to the most recent published FSA crop year** when the requested year is not out yet (reported via `data_year`, flagged in the UI and named in the source description), and its source description names the county/state found; legacy ambiguous-name rows are flagged "(state unknown)" with lookup disabled until re-added), ARC flat rates (fallback), payment limits; FSA base-acres import; **seed-cotton lint↔seed-cotton entry modes**; **per-year program parameters** (`program_year_config`: SCO trigger, per-person limit, sequestration %, plus OBBBA ARC guarantee/cap %, ERP factor/cap, payment factors). |
...
| `/reports/arc-plc-decision-aid` | **ARC/PLC Decision Aid** — compare projected PLC vs the **real ARC-CO county-revenue engine** (`computeArcCoPayment`: 90% guarantee on benchmark price × benchmark county yield, actual county revenue at max(MYA, loan), capped at 12% of benchmark revenue — parameters from `program_year_config`) per farm/commodity; set election. Counties without benchmark data fall back to the flat $/acre estimate, labeled. Surfaces the shared **MYA Prices panel** (incl. the USDA monthly-price AI lookup) + **County Yield Expectation** control (% vs benchmark ↔ absolute yield, persisted per commodity × county × year on `arc_benchmark_data`; counties resolve by `county_id` and display as "County, ST"); per-row **drivers** breakdown; the What-If MYA % slider now moves BOTH programs. OBBBA SCO note (SCO available regardless of election, 80% subsidy; 2025 pays higher-of automatically). |
| `/reports/government-payments` | **Government Payment Tracker** — projected ARC/PLC + other USDA payments with per-entity payment-limit tracking. **Year framing** (payment-year attribution): the default **"By payment year"** view shows the payments received in crop year Y — program year Y−1's ARC/PLC (header: "Y−1 program year → paid Oct Y") plus other payments landing in Y; a **"By program year"** toggle re-frames to the FSA program year for reconciliation (switching shifts the selected year ±1 so the same pool stays on screen). All math stays keyed to the program year; **payment limits stay per program year** (labeled). A one-time amber note flags other-payment entries that look like old program-year semantics. A visible **"ARC-CO settings: benchmarks & county yields →"** button deep-links to `/settings/government-payments#bench` (the settings page opens + scrolls the hashed section). A shared **MYA Prices panel** (`components/reports/mya-price-panel.tsx`, also on the Decision Aid) shows each commodity's resolved MYA with a state chip (**est. / manual / final**), its source (live Barchart / stored / operator override / USDA final), the effective reference price, and the **PLC payment-rate spread** (reference − MYA floored at 0) — plus an **Auto \| Manual toggle** (Manual persists an inline price to `arc_plc_price_data` with `source='manual'`; Auto reverts and clears it; untraded commodities are manual-only; a published final locks the row). The Details expander's monthly table has a **"Look up USDA prices"** button per commodity × marketing year (`/api/mya-monthly-lookup` — Claude web search over NASS monthly prices for the elapsed marketing-year months): fetched months render beside existing entries with differences highlighted, months already entered start **unchecked** (never overwritten without explicit confirmation), confirmed months save with `source='ai'` (an edited value becomes `'manual'`; typed entries save as `'manual'`), and the blend + "N of 12 months USDA-published" composition line recompute on save. Each estimate-mode commodity row also carries an **inline ↻ refresh** next to the MYA (both pages): it fires the same USDA lookup and shows a compact confirm strip under the row (fetched months + the resulting MYA previewed via `estimateMyaBlend`, confirm/cancel — same confirm-before-save contract); manual/final/manual-only rows show it disabled with a tooltip ("Switch to Auto to use lookups"). All projections update live on toggle/confirm. Exports carry the on-screen per-commodity **Election** (PLC/ARC-CO) column, the year-basis framing in the filter line, and the **Payment Limit Status** table (second section, status tones). |
...
**Supabase / PostgreSQL** with `pgcrypto` (`gen_random_uuid()`). Defined by **39 sequential,
idempotent migrations** in `supabase/` (`schema.sql` = 001, then `002_*.sql` … `039_*.sql`).
...
- **`mya_monthly_prices`** (037; `source` gains `'ai'` in 038) — USDA/NASS monthly national average farm prices per commodity × marketing year × calendar month, feeding the MYA blend estimate (`lib/mya-estimate.ts`; published months + futures-implied remainder × marketing weights). `source`: `'manual'` = operator-typed (incl. an edited AI value), `'ai'` = confirmed from the `/api/mya-monthly-lookup` autofill, `'usda'` = legacy rows. `covered_commodities` gains `mya_basis_adj` + `mya_month_weights`; `arc_plc_price_data` gains `wasde_midpoint` (overrides the blend within the estimate tier) + `mya_note`, with FSA-published 2025/2026 effective reference prices seeded (corn 4.42 / soy 10.71 / wheat 6.35) and OBBBA statutory prices/loan rates updated on `covered_commodities`.
...
- **Shared MYA resolution** (`lib/government-payments.ts`) — `resolveMyaPrice` is the ONE precedence for the MYA driving every PLC projection (**published final > manual override > live Barchart / stored estimate > missing**; untraded commodities are manual-only) and `applyMyaResolution` bakes it into effective `arc_plc_price_data` rows for `projectPayments`. Consumed by the Decision Aid, Payment Tracker, Revenue Projections, and Income Sensitivity, so all four project PLC from the same price. Live estimates come through one **`useLiveMya` hook** (`lib/use-live-mya.ts`) that resets on year change so a failed refetch can't leave last year's prices in play.
- **AI web-search lookups** (`lib/ai-web-search.ts` + `lib/ai-lookups.ts`) — the shared plumbing behind `/api/arc-benchmark-lookup` and `/api/mya-monthly-lookup` (a third lookup would be trivial): one web-search-enabled Claude call (`claude-sonnet-4-6`, `web_search_20250305` tool) returning parsed JSON, with an **in-process cache that also dedupes concurrent identical requests** (benchmarks 24 h on commodity + county_id + program year; monthly prices 30 min). `lib/ai-lookups.ts` is the pure, unit-tested layer — request validation (the benchmark lookup **rejects a missing state**), response normalization (only marketing-year months accepted; a null price forces `not_yet_published`; the benchmark `source_description` always names county + state), and the confirm-before-save merge (`defaultConfirmedMonths`/`planMonthlySaves`: manual wins unless explicitly confirmed; null months ignored; an edited AI value saves as `'manual'`). **Both lookups are confirm-before-save — the routes never write.**
...
- Package `@anthropic-ai/sdk ^0.95.2`; call sites: `app/api/parse-document/route.ts` (document
  ...
  `/api/arc-benchmark-lookup` and `/api/mya-monthly-lookup`).
...
| `/api/arc-benchmark-lookup` | `arc-benchmark-lookup/route.ts` | AI web-search autofill for FSA ARC-CO benchmark price/yield (county + state **required**; confirm-before-save; cached 24 h on commodity + county_id + year). | Anthropic |
| `/api/mya-monthly-lookup` | `mya-monthly-lookup/route.ts` | AI web-search autofill for USDA NASS monthly farm prices over the elapsed marketing-year months (confirm-before-save; cached 30 min). | Anthropic |
...
# In the Supabase SQL editor, run supabase/schema.sql, then 002…039 in order
...
- **Business logic:** `lib/*.ts` (pure functions — shrink, yields, contracts, hedging, marketing, crop-insurance, income-sensitivity, government-payments, program-config, revenue-projections, mya-estimate, ai-lookups, csv, exports, fuzzy, pdf-*; `ai-web-search.ts` is the one server-only AI helper). Unit tests live alongside as `lib/*.test.ts`.
