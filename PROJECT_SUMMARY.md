...
>
> _Snapshot date: 2026-07-08. Schema at migration `037`._
...
| Testing / CI | **Vitest `^4.1.8`** — 412 unit tests over the pure `lib/` math (14 `lib/*.test.ts` files); ESLint (`.eslintrc.json`, `next/core-web-vitals`); **GitHub Actions CI** (`.github/workflows/ci.yml`: lint + test on every push/PR) |
...
**Supabase / PostgreSQL** with `pgcrypto` (`gen_random_uuid()`). Defined by **37 sequential,
idempotent migrations** in `supabase/` (`schema.sql` = 001, then `002_*.sql` … `037_*.sql`).
...
- **`arc_benchmark_data`** (037) — FSA ARC-CO benchmark price & county benchmark yield per commodity × crop_year (× optional county; null county = default row), `price_source`/`yield_source` (usda/manual/ai), `county_yield_vs_benchmark_pct` (the persisted county-yield expectation, −30..+30), `source_description` (AI lookup provenance). Seeded 2025/2026 benchmark prices (corn 5.03 / soy 12.17 / wheat 6.98, source usda).
- **`mya_monthly_prices`** (037) — USDA/NASS monthly national average farm prices per commodity × marketing year × calendar month, feeding the MYA blend estimate (`lib/mya-estimate.ts`; published months + futures-implied remainder × marketing weights). `covered_commodities` gains `mya_basis_adj` + `mya_month_weights`; `arc_plc_price_data` gains `wasde_midpoint` (overrides the blend within the estimate tier) + `mya_note`, with FSA-published 2025/2026 effective reference prices seeded (corn 4.42 / soy 10.71 / wheat 6.35) and OBBBA statutory prices/loan rates updated on `covered_commodities`.
...
- **Business logic:** `lib/*.ts` (pure functions — shrink, yields, contracts, hedging, marketing, crop-insurance, income-sensitivity, government-payments, program-config, revenue-projections, csv, exports, fuzzy, pdf-*). Unit tests live alongside as `lib/*.test.ts`.
