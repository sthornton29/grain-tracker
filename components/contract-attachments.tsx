...
export default function ContractAttachments({ contractId }: { contractId: string }) {
  ...
  async function onPick(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (tooBig) { setErr(`"${tooBig.name}" is larger than 20 MB. Please use a smaller file.`); return }
    setBusy(true)
    try {
      const uploaded: ContractAttachment[] = []
      for (const file of files) {
        const { publicUrl, path } = await uploadFileToStorage(supabase, file, 'contract-attachments')
        const { data, error } = await supabase
          .from('contract_attachments')
          .insert({
    ...
    <div className="bg-white rounded-xl shadow p-4 space-y-3 print:hidden">
      ...
      {items == null ? (
        ...
        <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {items.map((a) => (
            <li key={a.id} className="border border-slate-200 rounded-lg p-2 flex gap-3 items-start bg-slate-50">
              <a href={a.file_url} target="_blank" rel="noreferrer" className="shrink-0" title={a.file_name}>
                {isImage(a.mime_type) ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={a.file_url} alt={a.file_name} className="w-20 h-20 object-cover rounded border border-slate-200" />
                ) : (
                  <div className="w-20 h-20 rounded border border-slate-200 bg-white flex items-center justify-center text-xs font-semibold text-slate-500">PDF</div>
                )}
              </a>
