...
Generated 2026-08-09 · version 0.1.0 · build a809e8f. Compiled from docs/help — regenerate with `npm run help:build`.
...
This page controls who can sign in and what they can see: invite new people, assign roles, and turn the Cotton module on or off. It's how you give your gin a place to key in loads without seeing your finances, or give a landlord read-only access to their own numbers.

## How to use it

- **Invite a user**: enter their email, pick a role, and either press **Send invite** (they get an email with a set-your-password link and land in your operation with that role) or press **Invite link** (no email is sent — you get a one-time link to copy and text or email yourself). Inviting a viewer requires picking at least one entity they may see.
...
- **Entity checkboxes** — which entities a viewer may see. Required for viewers; at least one must be picked.
...
- **Someone needs both cotton intake and reports.** Roles are one per user. Give them owner if you trust them with everything, or set up which access matters more — there's no combined gin-plus-viewer role.
