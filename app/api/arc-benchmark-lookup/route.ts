...
// Results are cached in-process on commodity + county_id + program year.
...
Set confidence to "high" ONLY if you found the number in FSA data or a source that clearly republishes FSA data for exactly this county/state/commodity/year. If you cannot find the county yield, return null for it — do NOT estimate, extrapolate from nearby counties, or use NASS county yields (those are not the ARC benchmark).

Respond ONLY with JSON, no other text, no markdown fences:
{"benchmark_yield": number or null, "benchmark_price": number or null, "source_description": "one or two sentences saying where the numbers came from, naming the county and state", "confidence": "high" or "low"}`
  ...
  try {
    ...
    return NextResponse.json({ data: normalizeArcBenchmarkResult(raw, { county, state }) })
  } catch (e) {
